...
<div class="d-flex align-items-start gap-3">
	...
	{{{ if (posts.annonymousType == "None") }}}
        ...
        <a class="d-inline-block position-relative text-decoration-none" href="" aria-label="[[aria:user-avatar-for, Anonymous]]">
            ...
            <span component="user/status" class="position-absolute translate-middle-y border border-white border-2 rounded-circle status offline"><span class="visually-hidden">[[global:offline]]</span></span>
        </a>
        {{{ end }}}
	</div>
	<div class="post-container d-flex flex-grow-1 flex-column w-100" style="min-width:0;">
		<div class="d-flex align-items-center gap-1 flex-wrap w-100 post-header mt-1" itemprop="author" itemscope itemtype="https://schema.org/Person">
			...
			{{{ if (posts.annonymousType == "None") }}}
    			<a class="fw-bold text-nowrap" href="{{{ if ./user.userslug }}}{config.relative_path}/user/{./user.userslug}{{{ else }}}#{{{ end }}}" data-username="{posts.user.username}" data-uid="{posts.user.uid}">{posts.user.displayname}</a>
			{{{ else }}}
    			<a class="fw-bold text-nowrap" href="" data-username="Anonymous-{posts.annonymousType}" data-uid="">{posts.annonymousType}</a>
			{{{ end }}}
